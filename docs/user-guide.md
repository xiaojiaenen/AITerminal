...
当前版本默认会阻止 AI 在对话模式里直接执行高风险命令。遇到需要人工批准的操作，请改用 `!命令` 或 `>描述` 模式确认后执行。
...
知识库会持久化到 `knowledge.store_path` 指定位置。默认配置如果给的是目录，程序会自动写入其中的 `knowledge.jsonl`。
...
| 知识库 | JSONL | `~/.ai-terminal/knowledge/knowledge.jsonl` | 是 | 目录配置会自动映射到该文件 |
...
24.5MB 左右主要来自 Python 运行时和全部依赖（wuwei + asyncssh + textual + rich + pydantic 等）。
